...
- The recovered RTL in [`08_recovered_rtl.v`](puzzle-solution/08_recovered_rtl.v) is one flat module, because that is what the netlist is: synthesis flattened the hierarchy and the layout keeps no record of it.
...
|---|---|---|
| **Plaisted-Greenbaum encoding** | Writes one direction of each definition when a gate is used in a single polarity, cutting the clause count by roughly half | It does not preserve the model set. Both interesting results here work by blocking a model and re-solving, which is sound only when models and circuit behaviours correspond one to one. Halving the formula and losing the two results it exists to produce is a bad trade |
| **AIG rewriting before encoding**, the ABC approach | Rebuilds the circuit as two-input ANDs and inverters, merges structurally identical nodes, then rewrites small windows into cheaper equivalents before any clause is written | Half of it is adopted, and is why the numbers above are what they are: the encoder folds a gate whose inputs are already constant and shares a gate identical to one already written, which took the puzzle formula from 130,385 variables to **14,498**. The rewriting half needs a full AIG package and at 728 cells would cost more than it saves |
...
- P13 writes a waveform with `success` high, which was not provided with the puzzle: [`puzzle-solution/14_success_inputs.vcd`](puzzle-solution/14_success_inputs.vcd).
...
| round | what it did | wall clock |
|---|---|---|
| twenty scripts, `yosys` per question | correct, and dominated by re-elaboration | ~240 s |
| one file, state kept in memory | no re-elaboration, but nothing else profiled | 17.0 s |
| first round of profiling | the polygon union, the coordinate transforms, the encoder, and sharding the equivalence run | 4.9 s |
| second round of profiling | the spatial query, the z3 encodings, the per-cell pin lookup, the shard count | **2.8 s** |
...
| P2, extract the puzzle | 4.87 s | 0.90 s | the polygon union, the coordinate transforms and the union-find |
...
| P2, extract the puzzle | 0.86 s | **0.47 s** | the same-layer query, the per-cell pin lookup, and the LEF parsed once |
...
| P10, the message enumeration | 0.40 s | **0.31 s** | the five example grids simulated in one bit-parallel pass instead of five separate ones |
| P11, RTL and the 564-grid equivalence | 1.95 s | **1.17 s** | z3 as above, and the shard count matched to physical cores |
...
|---|---|
| `Sum(If(...)) == k`, model read by evaluation | 1.38 s |
| `AtMost` + `AtLeast`, model read by lookup | **0.34 s** |
...
| shards | wall clock for the 564-grid run |
|---|---|
| 4 | 1.49 s |
| **8** | **1.01 s** |
| 12 | 1.08 s |
